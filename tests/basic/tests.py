...
class TestInvitationsSendView:
    ...
    def test_valid_form_submission(self, user_a):
        ...
        url = re.search("(?P<url>/invitations/[^\s]+)", mail.outbox[0].body).group(
            "url"
        )
        assert url == reverse(
...
class TestInvitationsAcceptView:
    ...
    def test_accept_invite_accepted_key(self, accepted_invitation, method):
        ...
        resp = client_with_method(
            reverse(
                "invitations:accept-invite", kwargs={"key": accepted_invitation.key}
            ),
            follow=True,
        )
        assert resp.status_code == 410
...
class TestInvitationSignals:
    ...
    def test_invite_url_sent_triggered_correctly(
        ...
        invite_url = reverse(
            "invitations:accept-invite", args=[sent_invitation_by_user_a.key]
        )
...
class TestInvitationsJSON:
    ...
    def test_anonymous_get(self):
        request = RequestFactory().get(
            reverse("invitations:send-json-invite"), content_type="application/json"
        )
